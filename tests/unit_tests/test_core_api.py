...
def test_app(notifier_function, db_mock):
    ...
    app = Flask(__name__)
    db_mock.get_data.return_value = {"categories": {"test-category": ["test-type-in-category"]}, "locations": [],
...
@mock.patch("goodmap.core_api.gettext", lambda x: f"{x}-translation")
def test_categories_endpoint_returns_categories(test_app):
    ...
    assert response.status_code == 200
    assert response.json == [["test-category", "test-category-translation"]]

@mock.patch("goodmap.core_api.gettext", lambda x: x)
@mock.patch("goodmap.formatter.gettext", lambda x: x)
@mock.patch("flask_babel.gettext", lambda x: x)
...
def test_getting_all_category_data(test_app):
    ...
    assert response.status_code == 200
    assert response.json == [['test-type-in-category', 'test-type-in-category-translation']]
