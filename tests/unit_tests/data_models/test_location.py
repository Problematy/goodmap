...
def test_proper_creation():
    location_model = create_location_model(
        obligatory_fields=[("used_obligatory_field", "str")], categories={}
    )
...
def test_missing_field_creation():
    location_model = create_location_model(
        obligatory_fields=[("not_used_obligatory_field", "str")], categories={}
    )
...
def test_create_location_model_backward_compatibility():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        model_str = create_location_model(obligatory_fields=[("name", "str")], categories={})
    ...
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        model_type = create_location_model(obligatory_fields=[("name", str)], categories={})
        assert len(w) == 1
...
def test_create_location_model_backward_compat_with_list():
    ...
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        model_str = create_location_model(obligatory_fields=[("tags", "list")], categories={})
        assert len(w) == 0
    ...
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        model_type = create_location_model(obligatory_fields=[("tags", list)], categories={})
        assert len(w) == 1
        assert issubclass(w[0].category, DeprecationWarning)
